...
Dando por concluido el presente laboratorio, se comprendió la complejidad y sensibilidad de los elementos, tales como los sensores, al momento de la captura de la señal de PPG. Por ello, se evidencia la necesidad de un estudio más profundo de las alternativas disponibles que permitan garantizar una adquisición de señal lo menos dependiente posible de factores externos.

Adicionalmente, se identificó que el sensor MAX30102, junto con el código de adquisición implementado, entregaba la señal invertida, lo cual afectó directamente la detección de picos y valles correspondientes a la sístole y diástole. Al comprender este comportamiento, fue posible corregir la inversión de la señal, permitiendo así una adecuada identificación de los valles y picos característicos. Esta corrección resultó fundamental para el cálculo preciso del SPI, ya que una detección errónea altera significativamente los resultados obtenidos.

Asimismo, se reafirmó la importancia de los modelos empleados para la medición del SPI, dado que la selección de un modelo inadecuado puede introducir errores en la estimación, afectando la interpretación de los resultados.

Por último, se comprendió la relación entre la medición del SPI y la actividad nociceptiva, tanto en condiciones de reposo como durante la activación simpática inducida por la maniobra CPT, e incluso en estados de analgesia. Tras la corrección en la adquisición y procesamiento de la señal, los valores del SPI obtenidos se encontraron dentro de rangos esperados durante la aplicación del CPT, lo que valida la metodología empleada. Finalmente, con el objetivo de mejorar la precisión en la evaluación de la nocicepción, se propone la integración de variables adicionales en futuros desarrollos, como las señales ANI y PI, capaces de complementar las limitaciones del SPI y reducir la incertidumbre asociada a factores fisiológicos propios de los pacientes.
